...
router.post("/analysis/spending", authMiddleware, async (req, res) => {
...
// 3. CẬP NHẬT: Khôi phục "trí nhớ" chi tiết từng giao dịch cho Chatbot
router.post("/assistant/chat", authMiddleware, async (req, res) => {
  ...
  try {
      // 🌟 BƯỚC 1: Sửa SQL để lấy CHI TIẾT từng giao dịch trong tháng này (gồm ngày, số tiền, ghi chú, danh mục, loại)
      const queryText = `
          SELECT 
      ...
      // 🌟 BƯỚC 2: Gom toàn bộ danh sách giao dịch chi tiết làm ngữ cảnh cho AI
      ...
      // 🌟 BƯỚC 3: Cấu hình hệ thống prompt (Đã thêm lệnh cấm dấu **)
      const model = ai.getGenerativeModel({
          model: "gemini-2.5-flash",
      ...
      // BƯỚC 4: Gửi dữ liệu sang Gemini kèm cơ chế tự động gọi lại nếu server bận (Retry Logic)
      let result;
      ...
      while (retries > 0) {
          try {
              ...
              // Nếu dính lỗi 503 (quá tải), giảm số lần thử và đợi 1.5 giây rồi thử lại
              if (geminiError.status === 503 && retries > 1) {
                  console.warn(`⚠️ Server Gemini đang bận, đang thử lại... (Còn ${retries - 1} lần thử)`);
                  retries--;
                  await new Promise(resolve => setTimeout(resolve, 1500)); // Đợi 1.5s
              } else {
                  throw geminiError; // Nếu lỗi khác hoặc hết lượt thử thì quăng lỗi ra ngoài
              }
          }
      }
      ...
      console.error("Lỗi Chatbot Gemini đọc chi tiết giao dịch:", error);
