...
const verifyToken = (req, res, next) => {
  ...
  if (!token) {
    return res.status(401).json({ error: "Bạn không có quyền truy cập, vui lòng đăng nhập trước." });
  }

  try {
    // Giải mã và kiểm tra token xem có hợp lệ / hết hạn không
    // Mẹo: Đảm bảo biến process.env.JWT_SECRET trong file .env trùng khớp với lúc tạo token khi Login nhé!
